...
@page { margin: 0; }
	...
	$sql ="SELECT *, room.price AS rprice, mealplan.price AS mprice, room.bedding AS rbed FROM payment LEFT JOIN room ON room.type=payment.TRoom LEFT JOIN mealplan ON mealplan.type=payment.Meal WHERE invoiceid = '$pid'";
	$query = $con->query($sql);
	$row=$query->fetch_assoc();
	$cpay = mysqli_num_rows($query);
		if ($cpay < 1) {
		echo "<script type='text/javascript'> alert('Payment Not Confirmed');
		...
		<article>
			...
			<table class="balance">
				...
				<tr>
					...
					<td><span data-prefix>NGN </span><span ><?php echo $amtpaid; ?></span></td>
				</tr>
				<tr>
			...
			<pre>
		<img src="../images/paid.png" height="120" width="120">
